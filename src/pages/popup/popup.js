...
function bindUI() {
    ...
    if (signInBtn) signInBtn.addEventListener("click", startDeviceFlow);
    ...
    if (saveBtn)
        saveBtn.addEventListener("click", () => onSave({ copyAfter: false }));
    const copyUrlBtn = $("copyUrlBtn");
    if (copyUrlBtn)
        copyUrlBtn.addEventListener("click", () => onSave({ copyAfter: true }));
...
function showMeta(data) {
    ...
    const fileOrg =
        (document.getElementById("fileOrg") &&
    ...
    if (owner && repo && data.id) {
        ...
        const langSel = document.getElementById("language");
        const chosenExt =
            langSel && langSel.value ? langSel.value : data.extension || "txt";
...
function onSave({ copyAfter = false } = {}) {
    ...
    if (!lastProblemData) {
        updateStatus("No detected problem. Click Detect first.", true);
        return;
    }

    if (!lastProblemData.code || lastProblemData.code.trim().length === 0) {
        const extra = lastProblemData.codeError
            ? ` Cause: ${lastProblemData.codeError}`
            : "";
        updateStatus(
    ...
    chrome.storage.local.get(
        ...
        (items) => {
            ...
            try {
                ...
                if (allowEl)
                    allowEl.addEventListener("change", persistPopupSettings);
                const showEl = document.getElementById("showBubble");
                if (showEl)
                    showEl.addEventListener("change", persistPopupSettings);
            } catch (e) {
                // ignore
            }
            ...
            try {
                ...
                console.warn(
                    "auto-detect on popup open failed",
                    e && e.message,
                );
            }
