...
// Helpers
function log(...args) {
  // prefix so logs are easy to find in SW console
  console.log('[bg | device-flow]', ...args);
}
function warn(...args) { console.warn('[bg | device-flow]', ...args); }
function error(...args) { console.error('[bg | device-flow]', ...args); }
...
function persistDeviceFlowState(state) {
  ...
  chrome.storage.local.set({ device_flow_state: state }, () => {
    log('persisted device flow state', state && { device_code: state.device_code, expiresAt: state.expiresAt });
  });
}
...
async function startDeviceFlow({ remember = false } = {}) {
  ...
  const { status, json, raw } = await postForm(DEVICE_CODE_URL, { client_id: CLIENT_ID, scope: 'repo' });
  log('device code response', { status, json });

  if (!json || !json.device_code) {
    throw new Error(`Device flow start failed: ${JSON.stringify(json || raw)}`);
  }
  ...
  scheduleAlarmForDeviceFlow(activeDeviceFlow.device_code, activeDeviceFlow.intervalSeconds);
...
async function pollForTokenOnce() {
  ...
  if (activeDeviceFlow.expiresAt && now >= activeDeviceFlow.expiresAt) {
    ...
    chrome.runtime.sendMessage({ action: 'deviceFlowExpired', message: 'Device flow expired. Please retry sign in.' });
    return;
  }
  ...
  if (json.error) {
    ...
    if (err === 'access_denied') {
      ...
      clearAlarmForDeviceFlow(code);
      activeDeviceFlow = null;
      chrome.runtime.sendMessage({ action: 'deviceFlowDenied', message: 'Authorization denied.' });
      return;
    }

    // unexpected error
    log('device flow error', json);
    chrome.runtime.sendMessage({ action: 'deviceFlowError', message: json.error_description || json.error || 'Unknown error' });
  ...
  if (json.access_token) {
    ...
    const masked = inMemoryToken ? (inMemoryToken.slice(0, 4) + '...' + inMemoryToken.slice(-4)) : null;
    ...
    try {
      notify('GitHub authorization', 'Signed in successfully. Return to CodeBridge to continue.');
    } catch (e) {
      log('notify failed', e && e.message);
    }
    chrome.runtime.sendMessage({ action: 'deviceFlowSuccess', tokenMasked: masked });
    return;
  }
    ...
    try {
      ...
      if (message.action === 'getAuthStatus') {
        const token = await (async () => {
          ...
          return new Promise((resolve) => {
            chrome.storage.local.get(['github_token'], (items) => resolve(items && items.github_token ? items.github_token : null));
          });
        })();
        const masked = token ? (token.slice(0,4) + '...' + token.slice(-4)) : null;
        sendResponse({ success: true, authenticated: !!token, tokenMasked: masked });
        return;
      }
      ...
      if (message.action === 'uploadFiles') {
        ...
        if (!owner || !repo || !files || files.length === 0) {
          sendResponse({ success: false, message: 'Missing owner/repo/files' });
          return;
        }
        try {
          const res = await uploadFilesToRepo({ owner, repo, branch, files, folder, allowUpdate });
          if (res.success) {
...
async function githubFetch(path, opts = {}, token) {
  ...
  const res = await fetch(`${GITHUB_API_BASE}${path}`, Object.assign({}, opts, { headers }));
  const text = await res.text();
  let json = null;
...
async function ensureRepoExists(owner, repo) {
  const getRes = await githubFetch(`/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}`, { method: 'GET' });
  ...
  if (getRes.status === 404) {
    ...
    throw new Error(`Failed to create repo: ${createRes.json && createRes.json.message ? createRes.json.message : JSON.stringify(createRes.json)}`);
  }
  throw new Error(`Failed to check repo: ${getRes.json && getRes.json.message ? getRes.json.message : JSON.stringify(getRes.json)}`);
}

async function getFileShaIfExists(owner, repo, path, branch) {
  const res = await githubFetch(`/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${encodeURIComponent(path)}?ref=${encodeURIComponent(branch || 'main')}`, { method: 'GET' });
  if (res.status === 200 && res.json && res.json.sha) return res.json.sha;
  if (res.status === 404) return null;
  throw new Error(`GitHub GET file failed: ${res.json && res.json.message ? res.json.message : JSON.stringify(res.json)}`);
}
...
async function putFile(owner, repo, path, base64Content, message, branch, sha) {
  ...
  throw new Error(res.json && res.json.message ? res.json.message : JSON.stringify(res.json));
}

async function uploadFilesToRepo({ owner, repo, branch = 'main', files = [], folder = '', allowUpdate = false }) {
  const token = await getTokenFromStorage();
  if (!token) return { success: false, message: 'Not authenticated with GitHub' };
  ...
  for (const f of files) {
    ...
    try {
      ...
      return { success: false, message: `Failed to check existing file ${path}: ${err.message}` };
    }
  }
  ...
  for (const f of files) {
    ...
    try {
      ...
      const message = (existingMap[path] ? `Update solution for ${path}` : `Add solution for ${path}`);
      const sha = existingMap[path];
      const json = await putFile(owner, repo, path, contentBase64, message, branch, sha);
      results.push({ path, url: json && json.content && json.content.html_url ? json.content.html_url : null });
    } catch (err) {
      return { success: false, message: `Failed to upload ${path}: ${err.message}` };
    }
  }
...
function notify(title, message) {
  try {
    if (chrome && chrome.notifications && chrome.notifications.create) {
      chrome.notifications.create({ type: 'basic', iconUrl: 'icons/icon.png', title, message });
    }
  } catch (e) { /* ignore */ }
}
...
(async function restoreOnStartup(){
  try {
    ...
    if (state && state.device_code) {
      log('restoring persisted device flow state on startup', { device_code: state.device_code });
      activeDeviceFlow = state;
      // ensure alarm exists
      scheduleAlarmForDeviceFlow(state.device_code, state.intervalSeconds || 5);
    }
  } catch (e) {
