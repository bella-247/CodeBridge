...
function notify(title, message) {
 ...
 // Some pages/platforms may prevent declarative content scripts from running in every frame.
 // To ensure the floating bubble appears on LeetCode pages, inject the content script
 // when a tab navigates or becomes active and the URL matches LeetCode.
     ...
     try {
         ...
         chrome.tabs.get(tabId, (tab) => {
             ...
             try {
                 ...
                 if (!url.startsWith("https://leetcode.com/")) return;
                 // Execute script in the tab's main world; try allFrames + MAIN world as a robust fallback.
                 // After injecting the file, run a small inline check that logs to the page console so users can
                 // see whether the bubble was created (helpful when content scripts are blocked in main frame).
                     try {
                     chrome.scripting.executeScript(
                         {
                         ...
                         (injectionResults) => {
                             if (chrome.runtime.lastError) {
                                 ...
                                 log("attempted injection of content script into tab", tabId, tab.url, { framesInjected: Array.isArray(injectionResults) ? injectionResults.length : null });
                             }
                             // Run a small diagnostic in the page to log whether the bubble DOM node exists.
                             try {
                                 chrome.scripting.executeScript(
                                     {
                                         target: { tabId },
                                         ...
                                         func: () => {
                                             try {
                                                 ...
                                                 // Use page console so the user can see it in DevTools
                                     ...
                                     (diagRes) => {
                                         if (chrome.runtime.lastError) {
                                             ...
                                             const bubbleExists = diagRes && diagRes[0] && typeof diagRes[0].result !== "undefined" ? !!diagRes[0].result : false;
                                             log("diagnostic injection executed", { bubbleExists });
                                             // If the bubble is not present, inject a minimal inline bubble as a last-resort fallback.
                                             if (!bubbleExists) {
                                                 try {
                                                     chrome.scripting.executeScript(
                                                         {
                                                             ...
                                                             func: () => {
                                                                 try {
                                                                     ...
                                                                     close.addEventListener("click", (ev) => {
                                                                         try { ev.stopPropagation(); wrapper.style.display = "none"; sessionStorage.setItem("lcgh-bubble-hidden", "1"); } catch (e) {}
                                                                     });
                                                                     ...
                                                                     // attach minimal click handler to message background (use postMessage to background if needed)
                                                                     wrapper.addEventListener("click", () => {
                                                                         try {
                                                                             console.log("lcgh: fallback-bubble clicked");
                                                                             // attempt to send a message via window.postMessage for content script consumers
                                                         ...
                                                         (fallbackRes) => {
                                                             if (chrome.runtime.lastError) {
                                                                 ...
                                                                 log("fallback injection executed", fallbackRes && fallbackRes[0] && typeof fallbackRes[0].result !== "undefined" ? { injected: !!fallbackRes[0].result } : {});
                                                             }
                                                         }
                                                     );
                                                 } catch (e) {
                                 ...
                                 log("diagnostic executeScript error", e && e.message);
                             }
                         }
                     );
                 } catch (e) {
 ...
 // Also attempt to inject to the currently active tab at startup as a best-effort
